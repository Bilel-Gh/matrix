...
def is_pivot_in_row(self, row: list, start_col: int = 0) -> bool:
    """
    Vérifie si un pivot existe dans la rangée à partir d'une colonne donnée.
    Un pivot est considéré comme existant si sa valeur absolue est supérieure à 1e-10.
    """
...
def row_echelon(self) -> 'Matrix':
    ...
    """
    result = Matrix([[self.values[i][j] for j in range(self.cols)] for i in range(self.rows)])
    ...
    while curr_row < result.rows and curr_col < result.cols:
        ...
        for row in range(result.rows):
            if row != curr_row and abs(result.values[row][curr_col]) > 1e-10:
                factor = result.values[row][curr_col]
                result.sub_row(result.values[curr_row], result.values[row], factor)
...
Matrix.find_best_pivot_row = find_best_pivot_row
Matrix.swap_rows = swap_rows
Matrix.div_row = div_row
Matrix.sub_row = sub_row